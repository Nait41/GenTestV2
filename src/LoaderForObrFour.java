...
public class LoaderForObrFour extends JFrame {
    XWPFDocument workbook;
    String nameObr;

    public LoaderForObrFour(String nameObr) throws IOException, InvalidFormatException {
    ...
    public void setPotentialForSecondTable(int numberTable, ArrayList<String> potentials){
        XWPFRun run = workbook.getTables().get(numberTable).getRow(2).getCell(0).getParagraphs().get(0).createRun();
        run.setFontSize(9);
        run.setFontFamily("Century Gothic");
        if(Double.parseDouble(potentials.get(0).replace(",", ".")) != 0){
            run.setText(String.format("%(.2f", Double.parseDouble(potentials.get(0).replace(",", "."))));
        } else {
            run.setText("0");
        }
        run = workbook.getTables().get(numberTable).getRow(2).getCell(1).getParagraphs().get(0).createRun();
        run.setFontSize(9);
        run.setFontFamily("Century Gothic");
        if(Double.parseDouble(potentials.get(1).replace(",", ".")) != 0){
            run.setText(String.format("%(.2f",Double.parseDouble(potentials.get(1).replace(",", "."))));
        } else {
            run.setText("0");
        }
        run = workbook.getTables().get(numberTable).getRow(2).getCell(2).getParagraphs().get(0).createRun();
        run.setFontSize(9);
        run.setFontFamily("Century Gothic");
        if(Double.parseDouble(potentials.get(2).replace(",", ".")) != 0){
            run.setText(String.format("%(.2f",Double.parseDouble(potentials.get(2).replace(",", "."))));
        } else {
            run.setText("0");
        }
    }
    ...
    public void setBioindexInLowInfo(InfoList infoList){
    ...
    public void setPielouEvenessInLowInfo(InfoList infoList){
    ...
    public void setGenusCountInLowInfo(InfoList infoList){
        int genusCount = 0;
    ...
    public void setPhylumRatioInLowInfo(InfoList infoList){
        ...
        XWPFRun run = workbook.getTables().get(1).getRow(6).getCell(2).getParagraphs().get(0).createRun();
        run.setFontSize(10);
        run.setFontFamily("Century Gothic");
        if(bact != 0 && firm != 0){
    ...
    public void setSpeciesRatioInLowInfo(InfoList infoList){
        double bf = 0, fp = 0;
        ...
        XWPFRun run = workbook.getTables().get(1).getRow(8).getCell(2).getParagraphs().get(0).createRun();
        run.setFontSize(10);
        run.setFontFamily("Century Gothic");
        if(fp != 0 && bf != 0){
    ...
    public void setFiveForFirstFormatTable(InfoList infoList, int numberTable) throws XmlException, IOException {
        ArrayList<ArrayList<String>> genusSpecies = new ArrayList<>();
        genusSpecies.addAll(infoList.genus);
        genusSpecies.addAll(infoList.species);
            ...
            for (int j = 0; j < genusSpecies.size(); j++)
            {
                if(workbook.getTables().get(numberTable).getRow(i).getTableCells().size() == 5)
                {
                    if(genusSpecies.get(j).get(0).equals(workbook.getTables().get(numberTable).getRow(i).getCell(1).getText())
                            ...
                            || genusSpecies.get(j).get(0).equals(workbook.getTables().get(numberTable).getRow(i).getCell(1).getText().
                            replace(" ", "_"))
                            || genusSpecies.get(j).get(0).equals(workbook.getTables().get(numberTable).getRow(i).getCell(1).getText().
                            replace("_", " "))
                    )
    ...
    public void saveFile(InfoList infoList, File docPath) throws IOException {
        workbook.write(new FileOutputStream(new File(docPath.getPath() + "\\" + infoList.fileName.replace(".xlsx", "") + "_выписка.docx")));
    }
}
