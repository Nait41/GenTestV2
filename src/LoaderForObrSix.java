...
public class LoaderForObrSix extends JFrame {
    XWPFDocument workbook;
    String nameObr;
    double firstPotential, secondPotential, thirdPotential;

    public LoaderForObrSix(String nameObr) throws IOException, InvalidFormatException {
    ...
    public void setPhylum(InfoList infoList) throws IOException, InvalidFormatException {
        ...
        workbook.getCharts().get(0).getWorkbook().getSheetAt(0).getRow(5).getCell(1)
                .setCellValue(otherCount);
        workbook.getCharts().get(0).replaceReferences(workbook.getCharts().get(0).getWorkbook().getSheetAt(0));
    }

    public void setRatioPhylum(InfoList infoList){
        double bact = 0, firm = 0;
        for (int j = 0; j < infoList.phylum.size(); j++)
        {
