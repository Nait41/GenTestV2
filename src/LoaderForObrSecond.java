...
public class LoaderForObrSecond extends JFrame {
    XWPFDocument workbook;
    String nameObr;
    double firstPotential, secondPotential, thirdPotential;

    public LoaderForObrSecond(String nameObr) throws IOException, InvalidFormatException {
    ...
    public void setPielouEveness(InfoList infoList){
        XWPFRun run = workbook.getTables().get(1).getRow(4).getCell(2).getParagraphs().get(0).createRun();
        run.setFontSize(12);
        run.setFontFamily("Century Gothic");
    ...
    public void setGenusCount(InfoList infoList){
        ...
        XWPFRun run = workbook.getTables().get(1).getRow(5).getCell(2).getParagraphs().get(0).createRun();
        run.setFontSize(12);
        run.setFontFamily("Century Gothic");
    ...
    public void setPhylum(InfoList infoList) throws IOException, InvalidFormatException {
        double otherCount = 0;
        for(int i = 1; i < workbook.getTables().get(2).getRows().size();i++){
            for (int j = 0; j < infoList.phylum.size(); j++)
            {
                if(workbook.getTables().get(2).getRow(i).getCell(0).getText().equals(infoList.phylum.get(j).get(0)))
                {
                    ...
                    run.setFontSize(9);
                    run.setFontFamily("Century Gothic");
                    run.setText(String.format("%(.2f", Double.parseDouble(infoList.phylum.get(j).get(1))));
                }
            }
            ...
            if (workbook.getTables().get(2).getRow(i).getCell(0).getText().contains("Firmicutes")){
                workbook.getCharts().get(1).getWorkbook().getSheetAt(0).getRow(1).getCell(1)
                        .setCellValue(Double.parseDouble(workbook.getTables().get(2).getRow(i).getCell(1).getText().replace(",", ".")));
            } else if (workbook.getTables().get(2).getRow(i).getCell(0).getText().contains("Bacteroidota")){
                workbook.getCharts().get(1).getWorkbook().getSheetAt(0).getRow(2).getCell(1)
                        .setCellValue(Double.parseDouble(workbook.getTables().get(2).getRow(i).getCell(1).getText().replace(",", ".")));
            } else if (workbook.getTables().get(2).getRow(i).getCell(0).getText().contains("Actinobacteriota")){
                workbook.getCharts().get(1).getWorkbook().getSheetAt(0).getRow(3).getCell(1)
                        .setCellValue(Double.parseDouble(workbook.getTables().get(2).getRow(i).getCell(1).getText().replace(",", ".")));
            } else if (workbook.getTables().get(2).getRow(i).getCell(0).getText().contains("Proteobacteria")){
                workbook.getCharts().get(1).getWorkbook().getSheetAt(0).getRow(4).getCell(1)
                        .setCellValue(Double.parseDouble(workbook.getTables().get(2).getRow(i).getCell(1).getText().replace(",", ".")));
            } else {
                otherCount+=(Double.parseDouble(workbook.getTables().get(2).getRow(i).getCell(1).getText().replace(",", ".")));
            }
        }
        workbook.getCharts().get(1).getWorkbook().getSheetAt(0).getRow(5).getCell(1)
                .setCellValue(otherCount);
        workbook.getCharts().get(1).replaceReferences(workbook.getCharts().get(1).getWorkbook().getSheetAt(0));
    }

    public void setRatioPhylum(InfoList infoList){
        ...
        XWPFRun run = workbook.getTables().get(3).getRow(1).getCell(2).getParagraphs().get(0).createRun();
        run.setFontSize(9);
        run.setFontFamily("Century Gothic");
        if(bact != 0 && firm != 0){
            run.setText(String.format("%(.3f",(bact/firm)));
        } else {
            run.setText("0");
        }
        run = workbook.getTables().get(3).getRow(2).getCell(2).getParagraphs().get(0).createRun();
        run.setFontSize(9);
        run.setFontFamily("Century Gothic");
        if(firm != 0 && prot != 0){
            run.setText(String.format("%(.3f",(firm/prot)));
        } else {
            run.setText("0");
        }
        run = workbook.getTables().get(3).getRow(3).getCell(2).getParagraphs().get(0).createRun();
        run.setFontSize(9);
        run.setFontFamily("Century Gothic");
        if(firm != 0 && acti != 0){
            run.setText(String.format("%(.3f",(firm/acti)));
        } else {
            run.setText("0");
        }
    }

    public void setRatioSpecies(InfoList infoList){
        double bf = 0, fp = 0;
        for (int j = 0; j < infoList.species.size(); j++)
        {
            if(infoList.species.get(j).get(0).equals("Bacteroides_fragilis"))
            {
                bf = Double.parseDouble(infoList.species.get(j).get(1).replace(",", "."));
            }
            if(infoList.species.get(j).get(0).equals("Faecalibacterium_prausnitzii"))
            {
                fp = Double.parseDouble(infoList.species.get(j).get(1).replace(",", "."));
            }
        }
        XWPFRun run = workbook.getTables().get(4).getRow(1).getCell(2).getParagraphs().get(0).createRun();
        run.setFontSize(9);
        run.setFontFamily("Century Gothic");
        if(bf != 0 && fp != 0){
            run.setText(String.format("%(.3f",(bf/fp)));
        } else {
            run.setText("0");
        }
    }
    ...
    public void setThreeDoubleFormat(InfoList infoList, int numberTable) throws IOException, InvalidFormatException {
        ...
        boolean checkFirst, checkSecond, checkThird;
        double potentialFirstNotNull = 0, potentialSecondNotNull = 0, potentialThirdNotNull = 0;
        for(int i = 1; i < workbook.getTables().get(numberTable).getRows().size();i++){
            ...
            for (int j = 0; j < genusSpecies.size(); j++)
            {
                ...
                {
                    potentialFirstNotNull+=Double.parseDouble(genusSpecies.get(j).get(1));
                ...
                if((workbook.getTables().get(numberTable).getRow(i).getCell(2) != null) && checkSecond)
                {
                    if((genusSpecies.get(j).get(0).equals(workbook.getTables().get(numberTable).getRow(i).getCell(2).getText())
                            || (genusSpecies.get(j).get(0).contains(workbook.getTables().get(numberTable).getRow(i).getCell(2).getText())
                            && genusSpecies.get(j).get(0).contains("/")
                            && !genusSpecies.get(j).get(0).contains(workbook.getTables().get(numberTable).getRow(i).getCell(2).getText() + "_"))
                            || workbook.getTables().get(numberTable).getRow(i).getCell(2).getText().equals(genusSpecies.get(j).get(0)
                            .replace("_", " "))
                            || workbook.getTables().get(numberTable).getRow(i).getCell(2).getText().equals(genusSpecies.get(j).get(0)
                            .replace(" ", "_"))) && checkSecond
                            && !workbook.getTables().get(numberTable).getRow(i).getCell(2).getText().equals("")
                    )
                    {
                        potentialSecondNotNull+=Double.parseDouble(genusSpecies.get(j).get(1));
                ...
                if((workbook.getTables().get(numberTable).getRow(i).getCell(4) != null) && checkThird)
                {
                    if((genusSpecies.get(j).get(0).equals(workbook.getTables().get(numberTable).getRow(i).getCell(4).getText())
                            || (genusSpecies.get(j).get(0).contains(workbook.getTables().get(numberTable).getRow(i).getCell(4).getText())
                            && genusSpecies.get(j).get(0).contains("/")
                            && !genusSpecies.get(j).get(0).contains(workbook.getTables().get(numberTable).getRow(i).getCell(4).getText() + "_"))
                            || workbook.getTables().get(numberTable).getRow(i).getCell(4).getText().equals(genusSpecies.get(j).get(0)
                            .replace("_", " "))
                            || workbook.getTables().get(numberTable).getRow(i).getCell(4).getText().equals(genusSpecies.get(j).get(0)
                            .replace(" ", "_"))) && checkThird
                            && !workbook.getTables().get(numberTable).getRow(i).getCell(4).getText().equals("")
                    )
                    {
                        potentialThirdNotNull+=Double.parseDouble(genusSpecies.get(j).get(1));
        ...
        double firstFormula, secondFormula, thirdFormula;
        firstFormula = (potentialFirstNotNull);
        secondFormula = (potentialSecondNotNull);
        thirdFormula = (potentialThirdNotNull);

        this.firstPotential = firstFormula;
        this.secondPotential = secondFormula;
        this.thirdPotential = thirdFormula;

        XWPFRun run = workbook.getTables().get(numberTable).getRow(workbook.getTables()
                .get(numberTable).getNumberOfRows()-1).getCell(0).getParagraphs().get(0).createRun();
        run.setFontSize(9);
        run.setFontFamily("Century Gothic");
        if(firstFormula != 0){
            run.setText(String.format("%(.2f",firstFormula));
        } else {
            run.setText("0");
        }
        run = workbook.getTables().get(numberTable).getRow(workbook.getTables()
                .get(numberTable).getNumberOfRows()-1).getCell(1).getParagraphs().get(0).createRun();
        run.setFontSize(9);
        run.setFontFamily("Century Gothic");
        if(secondFormula != 0){
            run.setText(String.format("%(.2f",secondFormula));
        } else {
            run.setText("0");
        }
        run = workbook.getTables().get(numberTable).getRow(workbook.getTables()
                .get(numberTable).getNumberOfRows()-1).getCell(2).getParagraphs().get(0).createRun();
        run.setFontSize(9);
        run.setFontFamily("Century Gothic");
        if(thirdFormula != 0){
            run.setText(String.format("%(.2f",thirdFormula));
        } else {
            run.setText("0");
        }

        workbook.getCharts().get(2).getWorkbook().getSheetAt(0).getRow(1).getCell(1)
                .setCellValue(firstFormula);
        workbook.getCharts().get(2).getWorkbook().getSheetAt(0).getRow(2).getCell(1)
                .setCellValue(thirdFormula);
        workbook.getCharts().get(2).getWorkbook().getSheetAt(0).getRow(3).getCell(1)
                .setCellValue(secondFormula);
        workbook.getCharts().get(2).getWorkbook().getSheetAt(0).getRow(4).getCell(1)
                .setCellValue(100 - firstFormula - secondFormula - thirdFormula);
        workbook.getCharts().get(2).replaceReferences(workbook.getCharts().get(2).getWorkbook().getSheetAt(0));
    }
    ...
    public void setFiveForSecondFormatTable(InfoList infoList, int numberTable, boolean checkPotential, boolean checkRandBacteria) throws XmlException, IOException {
        ArrayList<ArrayList<String>> genusSpecies = new ArrayList<>();
        genusSpecies.addAll(infoList.genus);
        genusSpecies.addAll(infoList.species);
        double potentialLow = 0, potentialMedium = 0, potentialHigh = 0, potentialNull = 0;
        for(int i = 0; i < workbook.getTables().get(numberTable).getRows().size();i++){
            if(workbook.getTables().get(numberTable).getRow(i).getCell(0).getText().contains("Случайные")){
                if(workbook.getTables().get(numberTable).getRows().size() - i - 4 > 0){
                    i++;
                    continue;
                } else {
                    break;
        ...
        if(checkPotential){
            ...
            if (resSum < 1.75){
                workbook.getTables().get(numberTable).getRow(workbook.getTables().get(numberTable).getNumberOfRows()-1)
                        .getCell(1).setText("ОЧЕНЬ НИЗКИЙ");
            } else if (resSum < 2.5){
                workbook.getTables().get(numberTable).getRow(workbook.getTables().get(numberTable).getNumberOfRows()-1)
                        .getCell(1).setText("НИЗКИЙ");
            } else if (resSum < 3.25){
                workbook.getTables().get(numberTable).getRow(workbook.getTables().get(numberTable).getNumberOfRows()-1)
                        .getCell(1).setText("СРЕДНИЙ");
            } else {
                workbook.getTables().get(numberTable).getRow(workbook.getTables().get(numberTable).getNumberOfRows()-1)
                        .getCell(1).setText("ВЫСОКИЙ");
            }
            */
            workbook.getTables().get(numberTable).getRow(workbook.getTables().get(numberTable).getNumberOfRows()-1)
                    .getCell(1).setText("НИЗКИЙ");
    ...
    public ArrayList<String> getPotentials(){
        ...
        potentials.add(String.format("%(.2f", firstPotential));
        potentials.add(String.format("%(.2f", secondPotential));
        potentials.add(String.format("%(.2f", thirdPotential));
        return potentials;
    }
